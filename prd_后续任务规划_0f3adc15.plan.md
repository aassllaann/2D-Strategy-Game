---
name: PRD 后续任务规划
overview: 以 [项目文档.md](d:/AI互动叙事游戏/项目文档.md)（v1.2.10）为基准，在现有 Godot 原型之上，补齐 **阶段A（P0 契约与核心规则）** 剩余项，再推进 **阶段B（演出与 HUD）**、**阶段C（结局/存档/设置）**，最后进入 **阶段D（内容与打磨）**。
todos:
  - id: schema-parser
  ...
  - id: hexagram-fx-layout
    content: "✅B1 变卦动画完成（变爻红闪→新卦翻入，移除六爻逐行高亮）。✅B3 布局与交互时序完成（顶部栏/三列/卦爻解析面板/三阶段/【提示】/手动存档按钮）。✅B3+ 页面布局深度重构（决策书卷全量展开/决策页居中浮层/选择后果页居中文本+解读按钮/因果推演页左卦象卡牌+右自适应分析文本）。⏸B2 HUD 危机表现（三才数值条 Tween/危机闪红/HUD 边框）尚待实现。"
    status: in_progress
