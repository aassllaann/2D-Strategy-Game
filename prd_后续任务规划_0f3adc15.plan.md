---
name: PRD 后续任务规划
overview: 以 [项目文档.md](d:/AI互动叙事游戏/项目文档.md)（v1.2.8）为基准，在现有 Godot 原型之上，补齐 **阶段A（P0 契约与核心规则）** 剩余项，再推进 **阶段B（演出与 HUD）**、**阶段C（结局/存档/设置）**，最后进入 **阶段D（内容与打磨）**。
todos:
  - id: schema-parser
  ...
  - id: endings-save-settings
    ...
    status: completed
  - id: m3-polish
    content: D1–D6：Intro 过渡动画、教程气泡、兜底库扩全量 64 卦、美术音频管线、Signal 解耦、测试与 KPI
    status: pending
isProject: false
---

# 基于《项目文档》的后续开发任务规划（v1.2.8 更新）
...
| **P10** | Intro 页面无场景过渡动画（I-03）                                                                                | FR-11       | **D1**         |
...
| 任务                            | 说明                                                                                                                                |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **D1. Intro 过渡动画（I-03）**  | 主菜单→Intro、Intro→游戏本体增加淡入淡出过渡，消除生硬直切。                                                                        |
| **D2. 情境式教程气泡（FR-12）** | 前 3 回合叠加提示气泡（回合 1 高亮战略区 / 回合 2 高亮卦象区 / 回合 3 高亮 HUD）；可点击关闭；`flags.tutorial_completed` 写入存档。 |
| **D3. 兜底库扩全量**            | 从 16 卦（192 条）扩至 64 卦×4×3=768 条（含完整 analysis）；可与美术并行。                                                          |
| **D4. 美术与音频**              | 每卦水墨背景（512×512px）、BGM 映射、音效资源；`hexagrams.json` 中 `background_image` / `bgm_track` 字段与资源管线一致。            |
| **D5. 模块通信规范**            | 核心跨模块通信统一走 Godot Signals，收敛直接跨场景节点引用；M3 代码审查消除技术债。                                                 |
| **D6. 测试与指标**              | AI 成功率≥95%、P50 响应≤3s、P95≤6s、崩溃率≤1%；无 Key 兜底完整一局；存档读写≤100ms。                                                |
...
| I-03 | Intro 页面无过渡动画，场景切换生硬            | 低     | 开放           | **D1**   |
